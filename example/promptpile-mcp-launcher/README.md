...
- **Playwright MCP**（[`@playwright/mcp`](https://www.npmjs.com/package/@playwright/mcp)）：已在 **`example/package.json`** 声明；`mcp.toml` 中为 **`npx -y @playwright/mcp --headless`**（无界面浏览器）。首次使用浏览器能力时可能自动下载浏览器二进制，体积与耗时较大。不需要 Playwright 时可删除 **`[servers.playwright]`** 整段以减轻启动负担。
