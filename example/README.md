...
Promptpile 自有示例已迁移至子模块 [`promptpile/examples/`](../promptpile/examples/)；本目录仅保留依赖 Hostra 生态的示例。
...
- `day-loom-init/`：`run-quick.bat` 验证 `day-loom init --quick`；`run-interactive.bat` 手跑 promptpile 访谈 init（需 `DEEPSEEK_API_KEY`）
