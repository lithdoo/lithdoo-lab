...
`fv.onTargetDirChange` 会触发 **`fv-state-changed`**（全量 `state`）。`fv.onFileChange` 会触发 **`fv-file-change`**，并在本地合并 `fileList` 后再触发 **`fv-state-changed`**。
...
- 监听 `<file-view>` 的 **`fv-state-changed`**，用 `detail.state` 重绘。
- **目录**格子：`click` / Enter / Space 时调用 **`fileView.setTargetDir(dir.fileUrl)`**（不写回 `target` 属性）。
