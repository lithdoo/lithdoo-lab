...
export async function runPromptpileUntilText(session: DailySession, baseUrl: string, token: string | undefined, maxToolRounds: number): Promise<string> {
  for (let round = 0; round <= maxToolRounds; round += 1) {
    const spawnConfig = getPromptpileSpawnConfig();
    const result = await runProcess(spawnConfig.command, [...spawnConfig.argvPrefix, '--config', path.basename(session.promptpileConfig), '-d', 'messages', '--tools-file', path.basename(session.toolsFile), '--continue', '--quiet'], { cwd: session.root, quiet: true });
    if (result.error) throw new Error(`Failed to start ${spawnConfig.displayName}: ${result.error.message}`);
