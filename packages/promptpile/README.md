...
| `PROMPTPILE_DEBUG` | 设为 `1` / `true` / `yes` / `on` 时向 **stderr** 输出额外诊断（已加载工具名、`tool_choice`、模型返回的 `tool_calls` 名称、`.calls.jsonl` 路径、after-hook 解析结果等）；**在 `-q` / `QUIET` 下仍会输出** | 关闭 |
...
| `-c, --continue` | 将本次 assistant 输出追加为下一条消息文件：有正文则写 `[N]assistant.md`；含 `tool_calls` 则写 `[N]assistant.call.jsonl`；两者**可共存**于同一 `N`，下一轮拼请求时会合并为一条 `{ role: 'assistant', content, tool_calls }`；都没有时不写文件 | 关闭 |
...
启用后会在 `directory` 根目录下追加 **`[N]assistant.md`**（本轮正文；若仅有工具调用而无正文，仍会写入该文件，可为空）。若本轮 API 返回 **`tool_calls`**，会追加 **同一 `N` 的 `[N]assistant.call.jsonl`**（每行一个 JSON，与 `outputs` 下的 `.calls.jsonl` 每行格式一致），便于下一轮 **[助理发起工具调用](#工具定义与历史工具调用toolstoml--toolsjsonl--assistantcall--assistantresult)**。主输出同目录下的 **`{basename}.calls.jsonl`** 仍仅在指定 **`-o` / `OUTPUT_FILE`** 时写入。
...
在 **API 成功返回** 且已完成 **主输出及 `{basename}.calls.jsonl`（若配置了 `-o`）**、以及可选的 **`--continue` 追加 `[N]assistant.md` / `[N]assistant.call.jsonl`** 之后，若解析到要运行的脚本文件，则 **`spawn` 子进程** 执行该文件（非交互、不经过 shell 拼接命令串）。**API 失败或中途 `exit(1)` 时不会执行钩子。**
...
| `PROMPTPILE_ASSISTANT_CALL_FILE` | 若启用了 `--continue` 且本轮含 `tool_calls`、写入了 `[N]assistant.call.jsonl` 则为该文件绝对路径，否则空字符串。两者**可同时非空**：下一轮拼请求时会被合并为单条 `{ role: 'assistant', content, tool_calls }` |
